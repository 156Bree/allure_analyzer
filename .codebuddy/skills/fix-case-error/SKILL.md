---
name: fix-case-error
description: This skill should be used when users ask to analyze a failed test case, especially pytest or Allure failures with simple assertion mismatches such as `AssertionError: assert 0 == 4`, and need a fast judgment on whether the issue is a False Positive caused by implement script error, polling timing, app-exit verification gaps, or test logic mistakes rather than a real product defect.
---
...
Classify common case failures quickly. Prioritize identifying False Positive patterns caused by implement script error, asynchronous polling mistakes, weak precondition checks, missing app-exit verification, stale cache, file lock, or cleanup timing problems.
...
- Case analysis requests that mention **False Positive**, **implement script error**, **script logic**, **polling**, **wait timing**, **app close verification**, **cleanup failure**, or **teardown lock**
...
5. Check whether the case assumed the app was closed without verifying that **all target apps had actually exited**.
6. Check whether cleanup, file replacement, cache, or file-lock behavior polluted the result.

Do **not** directly label the issue as a product defect only because the assertion failed. Escalate to product defect only after terminal-state polling, app-exit verification, and precondition verification still show the same wrong final behavior.
...
- any log lines around API request / API response / scan status / app close / teardown
...
- first suspicion: **polling / wait / assertion timing / app-exit verification / precondition logic**
...
- after closing apps, explicitly verify that **all target apps / sessions / foreground instances have exited** before asserting the close-related expectation
...
- **原因**: 断言时机过早、轮询缺失、App 关闭后未验证其已真正退出、前置校验不足、cleanup/file lock 干扰等
- **修复方向**: 改轮询、查终态、补 App 退出校验、补前置校验、增强日志
